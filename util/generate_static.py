...
def main(tx, canonical_fn, ofh):
  logging.info('starting...')

  canonical=set()
  for r in open(canonical_fn, 'rt'):
    canonical.add(r.strip().split('.')[0])

  logging.info('%i canonical transcripts', len(canonical))
  ...
  for r in csv.DictReader(gzip.open(tx, 'rt'), delimiter='\t'):
    if r['name'] in canonical:
      tx_name = '{} canonical'.format(r['name'])
    else:
      tx_name = r['name']
    odw.writerow({'gene': r['name2'], 'chrom': r['chrom'], 'tx': tx_name, 'txstart': r['txStart'], 'txend': r['txEnd'], 'genome': 'GRCh37'})

  logging.info('done')

if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='data for genome helper')
  parser.add_argument('--tx', required=True, help='transcripts')
  parser.add_argument('--canonical', required=True, help='canonical transcripts')
  ...
  main(args.tx, args.canonical, sys.stdout)
